            ...
            while(1) {
                ...
                switch (state) {
                    case HEAT_OFF:
                        ...
                        myLCD.cls();
                        myLCD.locate(0,0);
                        myLCD.printf("Temp=%5.2FF Heat",Current_tempF);
                        ...
                        else if ( Current_tempF <= cool_set ){
                            state = HEAT_ON;
                    ...
                    case HEAT_ON:
                        ...
                        else if ( Current_tempF >= heat_set ) {
                            state = HEAT_OFF;
                            mySpeaker.PlayNote(500,0.5,0.5);
                        }
                    break;
                    case COOL_OFF:
                        myLED4 = 0;
                        myShiftbrite.write(0,0,250);
                        setpoint_cursor = 14;
                        myLCD.cls();
                        myLCD.locate(0,0);
                        myLCD.printf("Temp=%5.2FF Cool\n",Current_tempF);
                        myLCD.locate(0,1);
                        myLCD.printf("Holding below %d", temp_set);
                        if ( mode == 0 ){
                            state = OFF;
                        }
                        else if ( mode == 1 ){
                            ...
                            state = COOL_ON;
                            mySpeaker.PlayNote(300.0,0.5,0.5);
                        }
                     break;
                    case COOL_ON:
                        myLED4 = 1;
                        ...
                        myLCD.cls();
                        myLCD.locate(0,0);
                        myLCD.printf("Temp=%5.2FF Cool",Current_tempF);
                        myLCD.printf("Cooling to %d   ", temp_set);   
                        if ( mode == 0 ){
                            state = OFF;
                        }
                        else if( mode == 1){
                            ...
                            state = COOL_OFF;
                            mySpeaker.PlayNote(400.0,0.5,0.5);
                        }
                    break;
                    case OFF:
                        myLCD.cls();
                        myLCD.locate(0,0);
                        myLCD.printf("Temp=%5.2FF Off",Current_tempF);
                        switch (mode){
                            case 0:
                                myLED3 = 0;
                                myLED4 = 0;
                            ...
                            case 1:
                                myLCD.cls();
                                state = HEAT_OFF;
                                myShiftbrite.write(255,165,0);
                            break;    
                            case 2:
                                myLCD.cls();
                                state = COOL_OFF;
                                myShiftbrite.write(0,0,205);
                            break;
                            }
                }
                wait(0.33);
